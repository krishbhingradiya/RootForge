...
async function runVoiceTests() {
  ...
  try {
    ...
    // Test Speech Processing with normal idea
    const speechResponse = await voiceWebhookService.processSpeech({
      callSid: 'CA_twiml_test_123',
      speechResult: 'I want to build an online grocery delivery app with fast search',
      sessionId: testSessionId
    });
