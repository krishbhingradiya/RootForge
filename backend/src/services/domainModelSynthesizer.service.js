/**
 * Domain Model Synthesizer Service
 * 
 * Provides dynamic, workspace-aware synthesis of the Canonical Domain Model:
 * Entities, Fields, Relationships, SQL DDL, Prisma Schema, REST APIs,
 * Integration Architecture Topology, Data Flow Sequences, and Multi-Rule Validation.
 * 
 * Ensures that the active workspace business requirement is the Single Source of Truth.
 */
...
/**
 * Domain Definitions Dictionary for deterministic generation when AI is offline/demo.
 * Every domain defines authentic business entities, foreign key relationships, REST APIs,
 * integration services, and data flows.
 */
    ...
    entities: [
      {
        ...
        fields: [
          { name: 'id', type: 'VARCHAR(36)', constraints: 'PRIMARY KEY', description: 'Physician UUID', isPrimaryKey: true, isNullable: false, isUnique: true, defaultValue: 'uuid_generate_v4()' },
      ...
      },
      {
        ...
        fields: [
          { name: 'id', type: 'VARCHAR(36)', constraints: 'PRIMARY KEY', description: 'Patient UUID', isPrimaryKey: true, isNullable: false, isUnique: true, defaultValue: 'uuid_generate_v4()' },
          ...
          { name: 'createdAt', type: 'TIMESTAMP', constraints: 'DEFAULT CURRENT_TIMESTAMP, NOT NULL', description: 'Intake timestamp', isPrimaryKey: false, isNullable: false, isUnique: false, defaultValue: 'CURRENT_TIMESTAMP' }
        ],
      ...
      },
      {
        ...
        fields: [
          ...
          { name: 'name', type: 'VARCHAR(100)', constraints: 'NOT NULL', description: 'Department name (e.g. Cardiology)', isPrimaryKey: false, isNullable: false, isUnique: false },
          { name: 'floorLocation', type: 'VARCHAR(50)', constraints: '', description: 'Building wing or floor', isPrimaryKey: false, isNullable: true, isUnique: false }
        ],
        sourceRequirements: ['FR-04']
      },
      {
        name: 'Appointment',
        description: 'Scheduled clinical consultations, slots, and encounter bookings.',
        fields: [
          { name: 'id', type: 'VARCHAR(36)', constraints: 'PRIMARY KEY', description: 'Appointment UUID', isPrimaryKey: true, isNullable: false, isUnique: true, defaultValue: 'uuid_generate_v4()' },
          { name: 'patientId', type: 'VARCHAR(36)', constraints: 'FOREIGN KEY, NOT NULL', description: 'References Patient.id', isPrimaryKey: false, isForeignKey: true, foreignTarget: 'Patient.id', isNullable: false, isUnique: false },
          { name: 'doctorId', type: 'VARCHAR(36)', constraints: 'FOREIGN KEY, NOT NULL', description: 'References Doctor.id', isPrimaryKey: false, isForeignKey: true, foreignTarget: 'Doctor.id', isNullable: false, isUnique: false },
          { name: 'scheduledTime', type: 'TIMESTAMP', constraints: 'NOT NULL', description: 'Scheduled appointment datetime', isPrimaryKey: false, isNullable: false, isUnique: false },
          { name: 'status', type: 'VARCHAR(30)', constraints: "DEFAULT 'SCHEDULED', NOT NULL", description: 'SCHEDULED, CHECKED_IN, COMPLETED, CANCELLED', isPrimaryKey: false, isNullable: false, isUnique: false, defaultValue: "'SCHEDULED'" },
          { name: 'createdAt', type: 'TIMESTAMP', constraints: 'DEFAULT CURRENT_TIMESTAMP, NOT NULL', description: 'Booking timestamp', isPrimaryKey: false, isNullable: false, isUnique: false, defaultValue: 'CURRENT_TIMESTAMP' }
        ],
    ...
    endpoints: [
      {
        ...
        description: 'Receives intake booking parameters, checks provider calendar conflicts, and locks the timeslot.',
        ...
        statusCodes: [{ code: 201, description: 'Appointment created successfully.' }, { code: 400, description: 'Timeslot conflict or invalid parameters.' }, { code: 401, description: 'Unauthorized JWT token.' }],
        errorResponse: JSON.stringify({ error: 'Conflict', message: 'Practitioner already has a consultation booked at this time.', code: 'ERR_TIMESLOT_CONFLICT' }, null, 2),
      ...
      },
      {
        ...
        statusCodes: [{ code: 200, description: 'Appointments returned.' }, { code: 401, description: 'Unauthorized.' }],
      ...
      },
      {
        ...
        purpose: 'Fetch patient master demographic profile and clinical encounter history.',
        ...
        statusCodes: [{ code: 200, description: 'Patient found.' }, { code: 404, description: 'Patient not found.' }],
        errorResponse: JSON.stringify({ error: 'Not Found', message: 'Patient record not found', code: 'ERR_PATIENT_NOT_FOUND' }, null, 2),
      ...
      },
      {
        ...
        description: 'Updates encounter status and records state change audit.',
        primaryEntity: 'Appointment',
        relatedEntities: [],
        authentication: 'Bearer JWT',
        authorization: 'Role: Staff, Practitioner',
        parameters: ':id (UUID)',
        requestBody: JSON.stringify({ status: 'CHECKED_IN', reason: 'Patient arrived' }, null, 2),
        responseBody: JSON.stringify({ id: 'apt_c08271fe', status: 'CHECKED_IN', updatedAt: '2026-10-15T09:25:00Z' }, null, 2),
        statusCodes: [{ code: 200, description: 'Status updated.' }, { code: 400, description: 'Invalid transition.' }],
        errorResponse: JSON.stringify({ error: 'Bad Request', message: 'Invalid transition', code: 'ERR_INVALID_TRANSITION' }, null, 2),
        sourceRequirements: ['FR-05']
      }
    ],
    primaryService: 'Clinical Orchestrator Service',
    externalProvider: 'SMS & Pager Gateway',
    ...
    entities: [
      {
        ...
        fields: [
          ...
          { name: 'createdAt', type: 'TIMESTAMP', constraints: 'DEFAULT CURRENT_TIMESTAMP, NOT NULL', description: 'Account creation date', isPrimaryKey: false, isNullable: false, isUnique: false, defaultValue: 'CURRENT_TIMESTAMP' }
        ],
    ...
    endpoints: [
      {
        ...
        description: 'Validates line items against product inventory, calculates order subtotal and taxes, and creates order records.',
        primaryEntity: 'Order',
    ...
    entities: [
      ...
      },
      {
        ...
        fields: [
          ...
          { name: 'capacityKg', type: 'DECIMAL(8,2)', constraints: 'NOT NULL', description: 'Payload payload capacity in kg', isPrimaryKey: false, isNullable: false, isUnique: false }
        ],
        sourceRequirements: ['FR-02']
      },
      {
...
/**
 * Determines domain family from workspace context
 */
export function identifyDomainFamily(context) {
  ...
  const combined = [
    ...
    ...(context?.discovery?.userStatements || [])
  ].join(' ').toLowerCase();
  ...
  return 'ECOMMERCE'; // Default fallback for commerce / enterprise
}

/**
 * Compiles PostgreSQL or SQLite SQL DDL script from normalized entities and relations
 */
export function compileSqlDdl(entities = [], relations = [], dialect = 'postgres') {
 ...
 */
export function compilePrismaSchema(entities = [], relations = []) {
  const lines = [];
...
/**
 * Derives multi-tier Integration Architecture nodes
 */
export function deriveDomainIntegrations(domain = 'Enterprise', entities = [], template = null) {
...
/**
 * Derives end-to-end Data Flow sequences
 */
export function deriveDomainDataFlow(domain = 'Enterprise', entities = [], endpoints = [], template = null) {
...
/**
 * Dynamic Multi-Rule Schema Validation Engine
 */
export function runValidationEngine(entities = [], relations = [], endpoints = [], integrations = []) {
...
/**
 * Main Entry Point: Synthesizes the Complete Canonical Domain Model
 */
export function synthesizeCanonicalDomainModel(context) {
  const ws = context?.workspace || context || {};
  const domainFamily = identifyDomainFamily(context);
  const template = DOMAIN_TEMPLATES[domainFamily] || DOMAIN_TEMPLATES.ECOMMERCE;

  const title = `${ws.name || domainFamily} ${template.titleSuffix}`;
  const entities = JSON.parse(JSON.stringify(template.entities));
  const relations = JSON.parse(JSON.stringify(template.relations));
  const endpoints = JSON.parse(JSON.stringify(template.endpoints));

  const sqlSchema = compileSqlDdl(entities, relations, 'postgres');
  const prismaSchema = compilePrismaSchema(entities, relations);
  const integrations = deriveDomainIntegrations(domainFamily, entities, template);
  const dataFlows = deriveDomainDataFlow(domainFamily, entities, endpoints, template);
  const validation = runValidationEngine(entities, relations, endpoints, integrations);
